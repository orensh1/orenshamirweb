// ============================================
// SITE CONTENT CONFIGURATION
// All editable text, links, and settings
// ============================================

export interface NavLink {
    name: string;
    href: string;
}

export interface FeatureCard {
    iconName: 'Smartphone' | 'Zap' | 'Layout';
    title: string;
    description: string;
    colorClass: string;
}

export interface ProcessStep {
    iconName: 'MessageCircle' | 'Code2' | 'Rocket';
    title: string;
    description: string;
    color: string;
    bg: string;
}

export interface FAQItem {
    question: string;
    answer: string;
}

export interface SiteContent {
    hero: {
        badgeText: string;
        headlineLine1: string;
        headlineLine2: string;
        subtitle: string;
        highlightWord: string;
        ctaText: string;
        waveColors: string[];
    };
    navbar: {
        logoText: string;
        links: NavLink[];
        ctaText: string;
    };
    about: {
        headingLine1: string;
        headingName: string;
        paragraphs: string[];
        features: FeatureCard[];
        imageAlt: string;
        imageCaption: string;
        imageSubCaption: string;
        marqueeText: string;
    };
    process: {
        titleBase: string;
        titleHighlight: string;
        steps: ProcessStep[];
    };
    faq: {
        titleBase: string;
        titleHighlight: string;
        items: FAQItem[];
    };
    contact: {
        headingLine1: string;
        headingHighlight: string;
        subtitle: string;
        email: string;
        phone: string;
        phoneDisplay: string;
        whatsappNumber: string;
        whatsappTitle: string;
        whatsappSubtitle: string;
        whatsappButtonText: string;
        instagramUrl: string;
        copyrightText: string;
        creditsText: string;
    };
    whatsapp: {
        phoneNumber: string;
        tooltipText: string;
    };
}

export const defaultContent: SiteContent = {
    hero: {
        badgeText: 'אורן שמיר | בונה דפי נחיתה מקצועיים',
        headlineLine1: 'לא עוד סתם',
        headlineLine2: 'דף נחיתה',
        subtitle: 'בלי סיפורים – אני בונה אתרים שעוזרים לכם להכניס יותר',
        highlightWord: 'כסף',
        ctaText: 'אני רוצה אתר כזה',
        waveColors: ['#3B82F6', '#06B6D4', '#A855F7', '#EC4899'],
    },
    navbar: {
        logoText: 'OSH',
        links: [
            { name: 'שירותים', href: 'process' },
            { name: 'קצת עליי', href: 'about' },
            { name: 'שאלות נפוצות', href: 'faq' },
        ],
        ctaText: 'רוצה אתר כזה?',
    },
    about: {
        headingLine1: 'נעים להכיר, ',
        headingName: 'אורן שמיר.',
        paragraphs: [
            'אני בן 16, מבאר שבע.',
            'אני בונה אתרים ודפי נחיתה לעסקים. אני אוהב את התחום הזה ומשקיע בכל פרויקט כדי שייצא הכי טוב שיש.',
            'העבודה איתי היא ישירה ופשוטה. בלי מילים מסובכות ובלי תהליכים ארוכים. אתם אומרים לי מה אתם צריכים, ואני דואג לבנות את זה.',
            'מוזמנים לשלוח הודעה ולהתחיל לעבוד.',
        ],
        features: [
            {
                iconName: 'Smartphone',
                title: 'נראה טוב בנייד',
                description: 'רוב הלקוחות מגיעים מהטלפון אז אני דואג שהאתר יעבוד שם חלק ומהיר',
                colorClass: 'purple',
            },
            {
                iconName: 'Zap',
                title: 'קוד נקי ומהיר',
                description: 'אני לא משתמש במערכות כבדות ומסובכות אלא בונה אתרים קלים ויעילים',
                colorClass: 'blue',
            },
            {
                iconName: 'Layout',
                title: 'מדברים איתי ישירות',
                description: 'בלי מנהלי תיקים ובלי להמתין לנציגים פשוט שולחים לי הודעה בוואטסאפ ומתקדמים',
                colorClass: 'pink',
            },
        ],
        imageAlt: 'Oren Shamir',
        imageCaption: 'אורן שמיר',
        imageSubCaption: 'מפתח & מעצב',
        marqueeText: 'OREN SHAMIR CREATIVE',
    },
    process: {
        titleBase: 'איך זה ',
        titleHighlight: 'עובד?',
        steps: [
            {
                iconName: 'MessageCircle',
                title: 'מבינים מה צריך',
                description: 'אנחנו מדברים בטלפון או בוואטסאפ ומחליטים מה בדיוק האתר צריך לעשות',
                color: 'text-blue-500',
                bg: 'bg-blue-500/10',
            },
            {
                iconName: 'Code2',
                title: 'אני בונה את האתר',
                description: 'אני ניגש לעבודה ובונה את הדף מאפס עד שהוא נראה מושלם ועובד חלק',
                color: 'text-purple-500',
                bg: 'bg-purple-500/10',
            },
            {
                iconName: 'Rocket',
                title: 'יוצאים לדרך',
                description: 'אחרי שאתם מאשרים שהכל נראה טוב אנחנו מחברים את הדומיין והאתר באוויר',
                color: 'text-pink-500',
                bg: 'bg-pink-500/10',
            },
        ],
    },
    faq: {
        titleBase: 'שאלות ',
        titleHighlight: 'נפוצות',
        items: [
            {
                question: 'האם זה בכלל מתאים לעסק שלי?',
                answer: 'אני מאמין שאתר דף נחיתה הוא חובה לכל עסק ישראלי שרוצה לצמוח,כיום כולם כולל כולם נמצאים באינטרנט ואם לא תיהיו באינטרנט תישארו מאחור',
            },
            {
                question: 'תוך כמה זמן הדף יהיה באוויר?',
                answer: 'הכי מהר שאפשר ברגע שיש לי את כל החומרים אני יושב על זה עד שזה מוכן בדרך כלל זה עניין של כמה ימים בודדים',
            },
            {
                question: 'מה קורה אם אני רוצה לשנות משהו אחרי שהדף עלה?',
                answer: 'פשוט מדברים איתי אני לא נעלם אחרי שהאתר באוויר שולחים לי הודעה בוואטסאפ ואני דואג לסדר את זה',
            },
            {
                question: 'למה לקחת דווקא אותך ולא חברה גדולה?',
                answer: 'בתור נער בן 16 יש לי הרבה יותר זמן פנוי מהחברות הגדולות כך שאוכל לייצר את האתר שלכם במלוא תשומת ליבי ובמהירות',
            },
        ],
    },
    contact: {
        headingLine1: 'בואו נתחיל ',
        headingHighlight: 'לעבוד.',
        subtitle: 'השאירו פרטים כאן או שלחו לי הודעה בוואטסאפ ונדבר על הפרויקט שלכם',
        email: 'orenshamir5@gmail.com',
        phone: '0526448826',
        phoneDisplay: '052-644-8826',
        whatsappNumber: '972526448826',
        whatsappTitle: 'זמין עכשיו בוואטסאפ',
        whatsappSubtitle: 'למענה מהיר ושיחה ישירה, שלחו לי הודעה.',
        whatsappButtonText: 'שלח הודעה',
        instagramUrl: 'https://www.instagram.com/orenshamir1/',
        copyrightText: '© 2026 Oren Shamir. All rights reserved.',
        creditsText: 'כל הזכויות שמורות לבונה האתר אורן שמיר 052-644-8826',
    },
    whatsapp: {
        phoneNumber: '972526448826',
        tooltipText: 'דבר איתי בוואטסאפ',
    },
};
